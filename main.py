...
async def check_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Check Promotions (Forwards / Bots / Spam Content / High Emoji Usage)
    if settings.get('delete_promotions', False):
        ...
        # C. Spam Text Analysis (Specific Keywords and Patterns)
        text_lower = message.text.lower()
        
        # Regex for common spam phrases (DM to buy, Price, Offers)
        spam_keywords_regex = r'(?i)(dm to buy|dm for|price\s*[\-:]|accounts\s*available|daily\s*offer|buy\s*for\s*resell|all\s*in\s*one\s*pack)'
        
        # Regex for Obfuscated Porn/Illegal keywords (CXP, P$RN, etc.) from user examples
        obfuscated_spam_regex = r'(?i)(c\s*x\s*p|p\s*[0o@$]\s*r\s*n|n\s*[u#]\s*d\s*e|r\s*[@a]\s*x\s*p\s*e|h\s*@\s*r\s*d|f\s*o\s*r\s*c\s*e\s*d|t\s*@\s*r\s*c\s*h\s*[u₹]\s*r)'
        
        # Regex for List format usually used in spam (e.g., 1️⃣ Full CXP...)
        list_spam_regex = r'(\d+[\ufe0f\u20e3\u0030-\u0039]\s*[\w\s/]+✅)'

        if re.search(spam_keywords_regex, text_lower):
            is_promotion = True
            reason = "promotional text"
        elif re.search(obfuscated_spam_regex, text_lower):
            is_promotion = True
            reason = "prohibited content"
        elif re.search(list_spam_regex, message.text):
            is_promotion = True
            reason = "spam list"

        # D. Emoji/Sticker Flood Detection
        ...
        # If emojis are > 30% of message OR total emojis > 15
        if emoji_count > 15 or (text_len > 10 and (emoji_count / text_len) > 0.4):
            is_promotion = True
            reason = "too many emojis"
        
        # If it matches any promotion criteria, delete it
        if is_promotion:
            try:
